...
public class SystemBarHelper {
    private static float DEFAULT_ALPHA = Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP ?  0 : 0.3f;
    ...
    public static void setStatusBarLightMode(Activity activity) {
        setStatusBarDarkMode(activity.getWindow(), false);
    }

    /**
     * 设置状态栏darkMode,字体颜色及icon变黑(目前支持MIUI6以上,Flyme4以上,Android M以上)
     */
    public static void setStatusBarDarkMode(Window window, boolean flag) {
        if (isFlyme4Later()) {
            setStatusBarDarkModeForFlyme4(window,flag);
        } else if (AppUtils.isMIUI()) {
            setStatusBarDarkModeForMIUI6(window, flag);
        }else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            setStatusBarDarkModeForM(window, flag);
        }
    }
    ...
    public static boolean isFlyme4Later() {
        return Pattern.compile("Flyme [4|5|6]", Pattern.CASE_INSENSITIVE).matcher(Build.DISPLAY).find()
