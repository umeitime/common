...
public abstract class AbstractNineGridLayout<T> extends ViewGroup {
    ...
    protected void fill(ViewGetter viewGetter) {
        ...
        for (int i = 0; i < MAX_CHILDREN_COUNT; i++) {
            ImageView imageView = (ImageView) viewGetter.getView(i);
    ...
    protected ImageView[] findInChildren(int viewId) {
        ImageView[] result = (ImageView[]) Array.newInstance(ImageView.class, getChildCount());
        for (int i = 0; i < result.length; i++) {
            ImageView imageView = getChildAt(i).findViewById(viewId);
    ...
    public abstract void render(T data, int itemWidth, int itemHeight);
