...
public class MultiImageView extends LinearLayout {
    ...
    public void setList(List<String> lists) throws IllegalArgumentException {
        ...
        if (MAX_WIDTH > 0) {
            pxMoreWandH = (MAX_WIDTH) / 3; //解决右侧图片和内容对不齐问题
    ...
    private void initImageLayoutParams() {
        ...
        int width = DisplayUtils.getScreenWidth(getContext())*2/5;
        onePicPara = new LayoutParams(width, width);
...
//            addView(new View(getContext()));
    ...
    private View createImageView(int position, final boolean isMultiImage) {
        ...
        Glide.with(getContext()).load(url).placeholder(R.drawable.default_pic).error(R.drawable.default_pic)
        ...
        if(imageViews.size()<imagesList.size())
